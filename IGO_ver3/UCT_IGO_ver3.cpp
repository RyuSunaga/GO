...
//selectによりRoot並列化を狙う
int UCT::SelectActionUCT_Paralelle2(int color, int loop = 1000) {
...
#pragma omp section
			{
				...
				if (this_thread == /*0*/5) {
					...
					for (int j = 0; j < ParentNode_Master->child_num; j++) {
						...
						//勝った回数を計算する
						win_count[this_thread][j] += ParentNode_Sub->child[j].win_rate * ParentNode_Sub->child[j].visit_count;
					}
...
#pragma omp section
	...
	//マスタースレッドの勝率を計算する
	//for (int j = 0; j < ParentNode_Master->child_num; j++) {
		//ParentNode_Master->child[j].win_rate = ((win_count[0][j] + win_count[1][j] + win_count[2][j] + win_count[3][j]) / ParentNode_Master->child[j].visit_count);
	//}
	...
	for (int j = 0; j < ParentNode_Master->child_num; j++) {
		//std::cout << ((win_count[0][j] + win_count[1][j] + win_count[2][j] + win_count[3][j]) / ParentNode->child[j].visit_count) << std::endl;
		ParentNode->child[j].win_rate = ((win_count[0][j] + win_count[1][j] + win_count[2][j] + win_count[3][j]) / ParentNode->child[j].visit_count);
	}
...
int UCT::playout(int start_color) {
	...
	return UCT::BM->WhichColorWin(start_color, false);
}
